...
my_sig = text_file_to_array("test_new.txt")
sig_fil = butterworth_filter(my_sig)
thresh = threshold(sig_fil)
positions = sample_positions_to_time(100, sig_fil, thresh)
peaks = r_peaks(sig_fil, thresh)
peak_dict = dict(zip(positions, peaks))
print(peak_dict)
norm_sig = normalise(sig_fil)
norm_dict = dict(zip(positions, norm_sig))
print(norm_dict)
...
#heart_monitor("ecg_hemal_1000.txt", 1000)
