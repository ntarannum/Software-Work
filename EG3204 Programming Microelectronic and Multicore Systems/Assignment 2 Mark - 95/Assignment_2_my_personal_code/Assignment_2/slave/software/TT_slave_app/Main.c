/*------------------------------------------------------------------*-
Modifications in the TT slave code by nt161:

-- changed the SharedMem_Update(void) in SharedMem_Mutex.c file to check only 3 positions of the receive buffer as position 0 holds the slave ID,
   the next two positions of the buffer hold the tick message data
-- in that same function, the if statement that checked the receive buffer position 4 was changed to position 2, as the master sends the code 0x06
   in the third byte of the tick message to turn on LED 5 on the TT slave.
