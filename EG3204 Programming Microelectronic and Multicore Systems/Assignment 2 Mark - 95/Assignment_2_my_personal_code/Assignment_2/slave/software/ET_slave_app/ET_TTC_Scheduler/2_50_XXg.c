...
tByte SCC_A_SLAVE_Process_Tick_Message(void)
{
	...
	if (Tick_ID == SLAVE_ID)
	{
		...
		Tick_message_data_G[2] = MCP2515_Read_Register(RXBnDm(1,3)); //comment out this line and the line below
		//Tick_message_data_G[3] = MCP2515_Read_Register(RXBnDm(1,4)); //because these two lines aren't being used anymore
